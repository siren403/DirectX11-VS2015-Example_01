...
bool SystemClass::Initialize()
{
	...
	//input객체를 생성합니다. 이 객체는 유저로부터 들어오는 키보드 입력을 처리하기 위해 사용합니다.
	...
	//graphics객체를 생성 합니다. 이 객체는 이 어플리케이션의 모든 그래픽 요소를 그리는 일을 합니다.
...
LRESULT CALLBACK SystemClass::MessageHandler(HWND hwnd, UINT umsg, WPARAM wparam, LPARAM lparam)
{
	switch (umsg)
	{
		...
		case WM_KEYDOWN:
		{
			//키가 눌렸다면 input객체에 이사실을 전달하여 기록하도록 합니다.
		...
		case WM_KEYUP:
		{
			//키가 떼어졌다면 input객체에 이사실을 전달하여 이 키를 헤제하도록 합니다.
